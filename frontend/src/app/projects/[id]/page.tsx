...
export default function ProjectPage() {
  ...
  const { currentProject, isLoading, error, fetchProject, markAsCompleted } = useProjectStore();
  ...
  useEffect(() => {
    const fetchProjectDetails = async () => {
      try {
        const projectData = await apiClient.fetchProject(params.id as string);
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newCommentResponse = await apiClient.addComment(params.id as string, comment);
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      ...
      // Make API call
      const response = await apiClient.toggleProjectLike(params.id as string);
        ...
        >
          <div className="overflow-hidden rounded-xl bg-white shadow-md dark:bg-gray-800">
            <div className="relative border-b bg-gradient-to-r from-blue-50 to-indigo-50 p-6 dark:border-gray-700 dark:from-blue-900/20 dark:to-indigo-900/20">
              ...
              <h1 className="text-2xl font-bold sm:text-3xl">{currentProject.title}</h1>
              <p className="mt-2 flex items-center text-sm text-gray-600 dark:text-gray-400">
                <Calendar className="mr-1 h-4 w-4" />
                Posted on {formatDate(currentProject.created_at)}
              </p>
            </div>
            ...
            <div className="overflow-hidden rounded-xl bg-white p-6 shadow-md dark:bg-gray-800">
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-medium ${currentProject.status === "OPEN"
                        ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400"
                        : "bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300"
                      }`}
                  >
