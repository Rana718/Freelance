...
export default function ProjectsPage() {
  const { projects, isLoading, error, filters, fetchProjects, setFilters, setPage, pagination } = useProjectStore();
  const { data: session } = useSession();
  const router = useRouter();

  const [techStack, setTechStack] = useState(filters.tech_stack || "");
  const [minBudget, setMinBudget] = useState(filters.min_budget?.toString() || "");
  const [maxBudget, setMaxBudget] = useState(filters.max_budget?.toString() || "");
  ...
  return (
    <div className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
      ...
      >
        <form onSubmit={handleFilterSubmit} className="flex flex-wrap md:flex-nowrap items-center gap-3 p-3 rounded-xl bg-white/80 dark:bg-gray-800/90 shadow-md">
      ...
      </motion.div>      {/* Project list or state */}
